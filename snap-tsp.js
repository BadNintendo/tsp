 ...
 * @returns {Object} - Returns the best-found path and distance.
 */
 const solveSatisfiabilityAndTSP = (clauses, cities) => {
    const memo = {};
     ...
     * @param {Object|Array} input - Path object {path, distance} or array of cities.
     * @returns {number} - The total distance.
     */
    const totalDistance = (input) => {
        const path = Array.isArray(input) ? input : input.path;
    ...
    const tspNearestNeighbor = (cities) => {
        ...
        while (unvisited.size > 0) {
            ...
            const nextCity = cities.reduce((closest, city) => {
                if (unvisited.has(city.name)) {
                    const dist = calculateDistance(currentCity, city);
                    if (!closest || dist < calculateDistance(currentCity, closest)) {
                        return city;
                    }
                }
                return closest;
    ...
    /**
     * Solves TSP using a genetic algorithm approach.
     * @param {Array} cities - Array of cities.
     * @param {number} populationSize - Size of the population.
     * @param {number} generations - Number of generations to evolve.
     * @returns {Object} - The best path and distance found.
     */
    const tspGeneticAlgorithm = (cities, populationSize = 100, generations = 1000) => {
        const randomPath = () => [...cities].sort(() => 0.5 - Math.random());
        const createPopulation = () => Array.from({ length: populationSize }, randomPath);
        const fitness = (path) => 1 / totalDistance(path);

        const select = (population) => {
            const weightedPopulation = population.map(path => ({
                path,
                weight: fitness(path)
            }));
            return weightedPopulation
                .sort((a, b) => b.weight - a.weight)
                .slice(0, 5)  // Top 5 fittest paths
                .map(item => ({ path: item.path, distance: totalDistance(item.path) }));
        };

        let population = createPopulation();
        for (let i = 0; i < generations; i++) {
            const selected = select(population);
            population = createPopulation(); // Create new population based on selected paths
        }
        
        // Ensure a valid result is returned
        const bestResult = select(population)[0];
        return bestResult ? bestResult : { path: [], distance: Infinity };
    };

    const nearestNeighborResult = tspNearestNeighbor(cities);
    const geneticAlgorithmResult = tspGeneticAlgorithm(cities);

    // Console output for results
    console.group("TSP Results with Starting Point 'A'");
    console.log("Nearest Neighbor Result:");
    console.log(`Path: ${nearestNeighborResult.path.map(city => city.name).join(" -> ")} | Distance: ${nearestNeighborResult.distance.toFixed(2)}`);
    console.log("Genetic Algorithm Result:");
    console.log(`Path: ${geneticAlgorithmResult.path.map(city => city.name).join(" -> ")} | Distance: ${geneticAlgorithmResult.distance.toFixed(2)}`);
    console.groupEnd();
};
...
solveSatisfiabilityAndTSP(clauses, cities);
